...
int mpi_count_friends_of_ten(int M, int N ,int **v) {
     ...
     if (rank > 0) {   
          ...
          int **subarray = allocate_2D_int(M, N_per_process + 3);

          
          MPI_Recv(&(subarray[0][0]), M * (N_per_process + 3), MPI_INT,
                   0, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

          friends_of_ten = count_friends_of_ten(M, N_per_process + 3, subarray);
